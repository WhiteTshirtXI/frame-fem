...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    methods
        ...
        function fi = calcInnerF(self, p_u, p_om)

            % allocate fi as zero-vector
            fi = zeros(self.sysDOF(),1);
            ...
            for el = self.elements'
                ...
                % calculate element matrices
                [ Me, Ke ] = matrices_beam( le, alpha, el{2} );
                ...
                g_idx_dof = self.NDOF*el{1}'-(self.NDOF-1);
                ...
                % calculate inner forces using the element matrices
                fi(idx_dof) = (-p_om^2*Me+Ke)*p_u(idx_dof);

                % using the sign convention for internal forces, switch
                % sign for every forces which acts on the first local
                % node of an element using this vector:
                signCorr = ones(2*self.NDOF,1);
                signCorr(1:2:end) = -1;

                fi(idx_dof) = signCorr.*fi(idx_dof);
